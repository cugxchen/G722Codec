...
int16_t WebRtcG722_EncoderInit(G722EncInst *G722enc_inst)
{
    ...
    G722enc_inst = (G722EncInst *) WebRtc_g722_encode_init((g722_encode_state_t *) G722enc_inst, 16000, 2);
    if (G722enc_inst == NULL)
    {
        return -1;
    }
...
int16_t WebRtcG722_Encode(G722EncInst *G722enc_inst, const int16_t *speechIn, int len, uint8_t *encoded)
{
    // Encode the input speech vector
    return WebRtc_g722_encode((g722_encode_state_t *) G722enc_inst, encoded, speechIn, len);
}
...
int16_t WebRtcG722_Decode(G722DecInst *G722dec_inst, const  uint8_t *encoded, int len, int16_t *decoded, int16_t *speechType)
{
    // Decode the G.722 encoder stream
    *speechType = G722_WEBRTC_SPEECH;
    return WebRtc_g722_decode((g722_decode_state_t *) G722dec_inst, decoded, encoded, len);
}
